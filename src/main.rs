...
fn main() {
        ...
        vulkano_shaders::shader! {
            ...
            src: "
				...
				void main() {
                    ...
                    double z_recip = 1.0 / zoom;
                    vec2 viewSize = vec2(2.0 * z_recip) * vec2(aspect, 1.0);

                    // screen to world space
                    vec2 xy = gl_FragCoord.xy / vec2(resolution);
                    xy = xy * viewSize - viewSize / 2.0;
                    xy += view_pos;
                    ...
                    float col = float(i) / float(num_iters);
					f_color = vec4(col, col, col, 1.0);
				}
			"
        }
    }
    ...
    let res = surface.window().inner_size().into();

    let mut push_constants = fs::ty::data {
        resolution: res,
        aspect:     res[0] as f64 / res[1] as f64,
        zoom:       1.0,
        num_iters:  100,
        view_pos:   [0f32, 0f32],
    };
    ...
    event_loop.run(move |event, _, control_flow| {
        match event {
            ...
            Event::WindowEvent {
                ...
                match scancode {
                    57419 => { // left arrow
                        push_constants.view_pos[0] -= 1.0 / push_constants.zoom as f32;
                    }
                    57421 => { // right arrow
                        push_constants.view_pos[0] += 1.0 / push_constants.zoom as f32;
                    }
                    57416 => { // up arrow
                        push_constants.view_pos[1] -= 1.0 / push_constants.zoom as f32;
                    }
                    57424 => {
                        push_constants.view_pos[1] += 1.0 / push_constants.zoom as f32;
                    }
            ...
            Event::WindowEvent {
                ...
                if state == ElementState::Pressed {
                    match button {
                        MouseButton::Left => { push_constants.zoom *= 2.0; }
                        MouseButton::Right => {push_constants.zoom *= 0.5; }
                        MouseButton::Middle => {}
                        MouseButton::Other(1) => {
                            push_constants.num_iters *= 2;
                            println!("num_iters: {}", push_constants.num_iters);
                        }
                        MouseButton::Other(2) => {
                            push_constants.num_iters /= 2;
                            println!("num_iters: {}", push_constants.num_iters);
                        }
                        MouseButton::Other(id) => { println!("{}", id); }
                    }
                    needs_redraw = true;
                }
            }
            Event::WindowEvent {
                ...
                push_constants.resolution = [new_size.width, new_size.height];
                push_constants.aspect = new_size.width as f64 / new_size.height as f64;
                needs_redraw = true;
            }
